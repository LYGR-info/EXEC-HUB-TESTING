...
/**
 * @fileOverview An AI agent that summarizes lengthy business reports.
 *
...
const SummarizeReportInputSchema = z.object({
  reportDataUri: z
    .string()
    .describe(
      "A business report, as a data URI that must include a MIME type and use Base64 encoding. Expected format: 'data:<mimetype>;base64,<encoded_data>'."
    ),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an expert business analyst. Please summarize the key findings of the following business report:\n\n{{media url=reportDataUri}}`,
});
